 ...
 */
public abstract class HBaseEntityMapper<T extends Entity>
    extends HBaseAbstractMapper {
  ...
  protected Permission getResourcePermission(Result result) {
    ...
    return new Permission(user, privilege);
  }
  ...
  protected void addQuestionComponentPut(
      List<Put> puts, Component component, String prefix) {

    QuestionComponent questionComponent = (QuestionComponent) component;
    String rowKey = String.format(ROW_KEY_FORMAT, prefix,
        COMPONENT_SEPARATOR, questionComponent.getId());
    Put put = new Put(Bytes.toBytes(rowKey));
    addTypeColumn(put, questionComponent.getClass().getSimpleName());
    addOrderColumn(put, questionComponent.getOrder());
    addScoreColumn(put, questionComponent.getScore());
    puts.add(put);
  ...
  protected void addCompositeQuestionDelete(
    ...
    Enumeration<Component> enumeration = composite.getComponents();

    while (enumeration.hasMoreElements()) {
      Component component = enumeration.nextElement();
      addQuestionComponentDelete(deletes, component, id);
    }
  }

  protected void addQuestionComponentDelete(
      List<Delete> deletes, Component component, String prefix) {
  ...
  protected void addCreatedByDelete(
    ...
    Auditable auditable = (Auditable) entity;
    User createdBy = auditable.getCreatedBy();
    String rowKey = String.format(ROW_KEY_FORMAT, entity.getId(),
        CREATED_BY_SEPARATOR, createdBy.getId());
    Delete delete = new Delete(Bytes.toBytes(rowKey));
    deletes.add(delete);
  }
  ...
  protected void addModifiedByDelete(
    ...
    Auditable auditable = (Auditable) entity;
    User modifiedBy = auditable.getModifiedBy();
    String rowKey = String.format(ROW_KEY_FORMAT, entity.getId(),
        CREATED_BY_SEPARATOR, modifiedBy.getId());
    Delete delete = new Delete(Bytes.toBytes(rowKey));
    deletes.add(delete);
  }
