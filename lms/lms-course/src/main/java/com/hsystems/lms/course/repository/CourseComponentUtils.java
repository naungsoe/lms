package com.hsystems.lms.course.repository;
...
public final class CourseComponentUtils {
  ...
  public static List<Component> organize(
    ...
    for (Nested<Component> component : components) {
      if (parentId.equals(component.getParentId())) {
        ...
        if (item instanceof TopicComponent) {
          TopicComponent topicComponent = (TopicComponent) item;
          List<Component> elements = organize(item.getId(), components);

          for (Component element : elements) {
            topicComponent.addComponent(element);
          }
        } else if (item instanceof ActivityComponent) {
